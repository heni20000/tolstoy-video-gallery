    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.upload.addEventListener("progress", (event) => {
        if (event.lengthComputable) {
          // Max 80% for upload (reserve 20% for server processing)
          const uploadPercentage = Math.round((event.loaded / event.total) * 80);
            ...
            {selectedFiles.map((file, index) => (
              <div key={index} className="bg-black-50 rounded-lg p-3">
                ...
                <div className="mt-1 text-xs">
                  {getProgressStatusText(progress[file.name])}
                </div>
              </div>
            ))}
          </div>
